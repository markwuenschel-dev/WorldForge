#!/usr/bin/env python3
"""validate_transition_contracts.py — v2.5 UE58TransitionForge contract-spine gate.

Proves the UE 5.7 -> 5.8 transition schema spine (transition_contracts.CONTRACTS) is
coherent and constrains correctly — the always-available, runtime-free gate that is GREEN
from Wave 1 while the conversion / plugin-build / regression / bridge / baseline gates stay
honestly RED until real 5.8 artifacts exist.
...
Reports -> procedural/reports/ue5_8/validate_transition_contracts_report.json
"""
...
def _registry_coherent(rep):
    rep.check("dogfood::registry_nonempty", len(TC.CONTRACTS) >= 7,
              "CONTRACTS registry must carry the 7 transition contracts",
              code=FailureCode.TRANSITION_REPORT_INTEGRITY_FAILED)
    ...
    rep.check("dogfood::known_bad_owning_complete",
              ...
              code=FailureCode.TRANSITION_REPORT_INTEGRITY_FAILED)
    rep.check("dogfood::owns_failure_band", len(TC.TRANSITION_CODES) >= 25,
              "transition milestone must own the WF1011-1060 failure band",
              code=FailureCode.TRANSITION_UNKNOWN_FAILURE_CODE)


def main(argv=None):
    ap = argparse.ArgumentParser(description="v2.5 transition contract-spine gate.")
    ...
    rep.set_meta(build_meta(
        command="transition-contracts", pack=args.pack, strict=strict,
        status=rep.status, record_count=n, records_total=n,
        report_type="wf.transition.contract_spine.v1",
        extra=engine_identity()))
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    rep.write(REPORT_DIR, "validate_transition_contracts_report.json")
    rep.print_summary("transition-contracts")
