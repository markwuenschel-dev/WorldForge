...
exit). Mirrors test_negative_recipes.py.

Runnable as a plain script (no pytest needed in CI); pytest-compatible too.

Usage:
    python tools/pipeline/test_negative_placement.py
    pytest tools/pipeline/test_negative_placement.py
"""
...
FIXTURES_DIR = REPO_ROOT / "tests" / "fixtures" / "invalid_placement"
VALIDATOR = REPO_ROOT / "tools" / "pipeline" / "validate_placement.py"
...
def _run_validator(fixture: Path) -> subprocess.CompletedProcess:
    return subprocess.run(
        [sys.executable, str(VALIDATOR), "--definition-path", str(fixture)],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
...
def _assert_rejected(fixture: Path) -> None:
    ...
    assert result.returncode != 0, (
        ...
        f"but validate_placement.py exited 0 (accepted it).\n"
        f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )


def test_all_invalid_placement_rejected() -> None:
    fixtures = _fixtures()
    assert fixtures, f"No invalid-placement fixtures found under {FIXTURES_DIR}"
...
def main() -> int:
    ...
    fixtures = _fixtures()
    if not fixtures:
        print(f"ERROR: no invalid-placement fixtures found under {FIXTURES_DIR}", file=sys.stderr)
        return 2
