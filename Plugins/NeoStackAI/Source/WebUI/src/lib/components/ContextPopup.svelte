...
{#if visible}
	<div
		bind:this={popupEl}
		class="border-border/60 absolute bottom-full left-0 z-50 mb-1 max-h-[280px] w-[380px] overflow-y-auto rounded-lg border bg-surface-popup py-1 shadow-xl"
	>
		{#if isLoading && results.length === 0}
			<div class="text-muted-foreground/40 px-3 py-2 text-[13px]">Searching...</div>
		{:else if results.length === 0}
			<div class="text-muted-foreground/40 px-3 py-2 text-[13px]">No results</div>
		{:else}
			{#each flatResults as item, idx (`${item.type}:${item.path}`)}
				<button
					data-index={idx}
				...
				>
					<span class="context-icon flex h-4 w-4 shrink-0 items-center justify-center opacity-70">
						{#if item.type === 'folder'}
							...
							>
								<path
									d="M1.5 3C1.5 2.44772 1.94772 2 2.5 2H6.29289C6.4255 2 6.55268 2.05268 6.64645 2.14645L8 3.5H13.5C14.0523 3.5 14.5 3.94772 14.5 4.5V13C14.5 13.5523 14.0523 14 13.5 14H2.5C1.94772 14 1.5 13.5523 1.5 13V3Z"
									fill="white"
									fill-opacity="0.5"
								/>
							</svg>
						{:else}
							...
							>
								...
								/>
								<path
									d="M9.5 1L13 4.5H10C9.72386 4.5 9.5 4.27614 9.5 4V1Z"
									fill="white"
									fill-opacity="0.6"
								/>
							</svg>
					...
					<span class="truncate {item.type === 'folder' ? 'text-foreground/90' : ''}"
						>{item.name}</span
					>
				</button>
			{/each}
